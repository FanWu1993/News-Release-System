...
public class InsertType extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
	    ...
	    //判断输入的类别是否已经存在
	    try{
	      ResultSet rs = con.executeQuery("select * from tb_newsType where TypeName='" + typeName + "'");
	     if (rs.next()) {
	        out.println("您添加的类别已经存在，请输入其它类别！");
	        out.println("<a href=../news/manager/insertType.jsp>" + "添加" + "</a>");
	      }
	      else {
	          String sql = "insert into tb_newsType values('" + typeName + "','" +dateTime + "')";
	          con.executeUpdate(sql);
	          response.sendRedirect("/news/manager/insertTypeSuccess.jsp");
	      }
	    }
